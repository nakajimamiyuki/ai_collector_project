...
[![tests-count](https://img.shields.io/badge/pytest-81%20passed-brightgreen.svg)](#-测试)
[![MCP](https://img.shields.io/badge/MCP-3%20tools-purple.svg)](docs/MCP_SERVER.md)
...
> 整套能力通过 MCP Server 暴露 3 个工具（字面检索 / 语义检索 / 技能缺口分析），
> 任何 MCP 客户端可调用；每次运行自动落 Bad Case 库，形成"跑 → 复盘 → 修 → replay"闭环。
...
> 💡 **简历/PPT 用导出 PNG**：macOS 用户在 Preview 打开 `docs/assets/architecture.svg`，
> `Cmd+P` → 另存为 PDF；或 `qlmanage -t -s 1920 -o /tmp docs/assets/architecture.svg` 输出 PNG。
>
> 🎯 **面试准备**：项目能力对照 3 家目标 JD 的关键词矩阵见 [`docs/JD_MAPPING.md`](docs/JD_MAPPING.md)
> （含话术模板 + 真问真答预演 + 投递策略 + 自查清单）。

> 🆕 **v3.1** (2026-06-26)：MCP Server 3 Tools / Bad Case 闭环 / 81 pytest 单测
...
| **v3.1** | 把求职 Agent 接进 MCP 生态（3 个 Tool + Bad Case 闭环 + 简历级打磨） | [📖 在 CSDN 阅读](https://blog.csdn.net/peaceworld_/article/details/162350620) | [`docs/blog/v3.1_*.md`](docs/blog/v3.1_把求职Agent接进MCP生态.md) |
...
- 🔗 **v3.1 起步：MCP Server**：基于 FastMCP 暴露 3 个 Tool（`search_jobs` 字面检索 + `query_rag` 语义检索 + `get_skill_gap` 技能缺口分析），Claude/Cursor/Hermes 可通过 stdio 调用本地 192 条 Boss JD 数据
...
```bash
# 当前 3 个 Tool：
#   - search_jobs(keyword, city, top_k)       字面包含匹配
#   - query_rag(question, top_k)              bge-m3 + Milvus 语义检索
#   - get_skill_gap(top_n)                    市场技能热度 + 个人缺口对照
...
async def main():
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # 字面检索
            r = await session.call_tool('search_jobs', {'keyword': 'MCP', 'city': '杭州', 'top_k': 2})
            print(r.content[0].text)
            # 语义检索（需要 ollama 在跑 + vector.db 已建）
            r = await session.call_tool('query_rag', {'question': 'LangGraph 反思决策 Agent', 'top_k': 2})
            print(r.content[0].text)
            # 技能缺口分析
            r = await session.call_tool('get_skill_gap', {'top_n': 8})
            print(r.content[0].text)
...
详细配置（macOS OpenMP 坑 / Hermes / Claude Desktop）见：[`docs/MCP_SERVER.md`](docs/MCP_SERVER.md)

**E. v3.1 Bad Case 闭环（让 Agent 越用越准）**：

每次跑 `find_jobs.py` 都会自动落一条记录到 `data/agent_runs.db`，**零结果自动标 bad**。
人工 review 时可以打 root_cause（`filter_too_strict` / `rag_miss` / `prompt_off` ...），
修完之后用 `replay` 命令把所有 bad case 重跑一遍验证是否真的修好了。

```bash
# 看最近所有跑过的查询（id / 时间 / 结果数 / 用时 / 反思次数 / status / 根因 / query）
python scripts/agent_runs.py list

# 只看失败的
python scripts/agent_runs.py list --status bad

# 看某条详情（query / trace / 完整报告 / 根因 / 修复 commit）
python scripts/agent_runs.py show 2

# 人工 review：打根因 + 关联 fix commit
python scripts/agent_runs.py mark 2 --status bad \
    --root-cause filter_too_strict \
    --fix-commit abc1234 \
    --fix-notes "把 salary_min 从 20K 降到 15K"

# 修完之后批量 replay 所有 bad case，看是不是真修好了
python scripts/agent_runs.py replay --status bad --limit 5

# 总览：unreviewed / good / bad 各多少
python scripts/agent_runs.py stats
```

设计原则：
- `agent_runs.db` 跟 `collector.db` 物理隔离 —— v1/v2 pipeline 的状态机不污染
- 零结果 = 立即 bad（不用人工先发现），降低 review 摩擦
- replay 不会覆盖老记录，新跑写新行，便于做 A/B 对比
...
│       ├── bad_case_store.py     # ★ v3.1 Bad Case 闭环存储（agent_runs.db）
│       └── my_profile.yaml       # 用户画像（学历/年限/技能/avoid）
│
├── src/mcp_server/               # ★ v3.1 MCP Server
│   └── ai_collector_mcp.py       # 3 个 Tool: search_jobs / query_rag / get_skill_gap
│
├── scripts/                      # CLI 工具集
...
│   ├── find_jobs.py              # ★ v3.0 Agent CLI 入口（跑完自动落 agent_runs.db）
│   └── agent_runs.py             # ★ v3.1 Bad Case CLI: list/show/mark/replay/stats
│
